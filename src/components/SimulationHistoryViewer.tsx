          ...
          <div className="p-8">
            ...
            {activeTab === 1 && (
              <div className="space-y-6">
                ...
                {results.subFunctions && Array.isArray(results.subFunctions) ? (
                  <div className="space-y-4">
                    {results.subFunctions.map((sf: any, index: number) => (
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  <div className="p-6 bg-cyan-50 rounded-lg border border-cyan-200">
                    <div className="text-sm text-cyan-700 mb-1">Total Recommended FTE</div>
                    <div className="text-3xl font-bold text-cyan-900">{simulation.total_fte || 'N/A'}</div>
                  </div>
                  ...
                  <div className="p-6 bg-gray-50 rounded-lg border border-gray-200">
                    <div className="text-sm text-gray-700 mb-1">Workload Score</div>
                    <div className="text-3xl font-bold text-gray-900">{simulation.workload_score || 'N/A'}</div>
                  </div>
                </div>

                {results.subFunctions && Array.isArray(results.subFunctions) && (
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Sub-Function Breakdown</h3>
                    <div className="space-y-3">
                      {results.subFunctions.map((sf: any, index: number) => (
                        <div key={index} className="border border-gray-200 rounded-lg p-4 bg-white hover:shadow-sm transition-shadow">
                          <div className="flex items-center justify-between mb-2">
                            <h4 className="font-semibold text-gray-900">{sf.name}</h4>
                            {sf.recommendedFTE && (
                              <span className="text-sm font-medium text-cyan-700">
                                {sf.recommendedFTE.min}-{sf.recommendedFTE.max} FTE
                                (Target: {sf.recommendedFTE.recommended})
                              </span>
                            )}
                          </div>
                          {sf.currentHeadcount !== undefined && (
                            <div className="text-sm text-gray-600">
                              Current Headcount: {sf.currentHeadcount} FTE
                            </div>
                          )}
                        </div>
                      ))}
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-6">
                  <p className="text-sm text-yellow-800">
                    <strong>Note:</strong> This is a saved simulation result. All fields are read-only.
                  </p>
                </div>
              </div>
            )}
